...
fn fmt(matches: &ArgMatches) {
    ...
    let pool: ThreadPool<FormatPayload> =
        ThreadPool::spawn(thread_count, thread_count * 2, |payload: FormatPayload| {
...
fn format_target(target_path: &Path, criteria: &TargetSearchCriteria) {
...
fn print_final_status(elapsed_ms: i64) {
    ...
    logger::info(format!(
        ...
        elapsed_ms, total, unchanged_msg, formatted_msg, failed_msg
    ));
}
